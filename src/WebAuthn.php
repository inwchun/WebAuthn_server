...
/**
 * Modified version of https://github.com/lbuchs/WebAuthn/blob/master/src/WebAuthn.php
 * WebAuthn
 * @author Lukas Buchs
 * @license https://github.com/lbuchs/WebAuthn/blob/master/LICENSE MIT
 * Modified By Inwhan Chun
 */
class WebAuthn {
    // relying party
    private $_rpName;
     ...
     * @return \stdClass
     */
    public function getGetArgs($credentialIds, $timeout=20, $allowUsb=true, $allowNfc=true, $allowBle=true, $allowInternal=true, $requireUserVerification=false) {
    ...
    public function processCreate($clientDataJSON, $attestationObject, $challenge, $requireUserVerification=false, $requireUserPresent=true, $failIfRootMismatch=true) {
        ...
        $data = new \stdClass();
        $data->cid = $attestationObject->getAuthenticatorData()->getCredentialId();
        $data->pk = $attestationObject->getAuthenticatorData()->getPublicKeyPem();
    ...
    public function processGet($clientDataJSON, $authenticatorData, $signature, $credentialPublicKey, $challenge, $prevSignatureCnt=null, $requireUserVerification=false, $requireUserPresent=true) {
        ...
        if ($this->verify($publicKey, $dataToVerify, $signature) !== 1) {
            throw new WebAuthnException('invalid proof', WebAuthnException::INVALID_SIGNATURE);
        }
    ...
    /**
     * verify proof of HanPass
     * @param resource $vk public key
     * @param string $m signed message for WebAuthn
     * @param string $pi proof
     * @return int 1 ifsuccessful
     */
    private function verify($vk, $m, $pi) {
        $ITERATIONS = 4096;
        $details = openssl_pkey_get_details($vk);
        $x = $details["ec"]["x"];
        $y = $details["ec"]["y"];
        // $dataToVerify = $m;
        $sigLength = ord($pi[1]) + 2;
        $signature = substr($pi, 0, $sigLength);
        $t_pzl = substr($pi, $sigLength, 32);
        $r = substr($pi, $sigLength + 32);
        // $p = null;

        for ($i=0; $i < $ITERATIONS; $i++) {
            $p_ = pack('n', $i);
            if($t_pzl == openssl_pbkdf2($x.$y.$p_, $r, 32, 25, 'sha256')){
                if(\openssl_verify(openssl_pbkdf2($m.$x.$y.$p_, $r, 32, 25, 'sha256'), $signature, $vk, OPENSSL_ALGO_SHA256) == 1)
                    return 1;
            }
        }
        return 0;
